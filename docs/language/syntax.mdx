...
A <StyledFlowLog /> program is a plain-text <code>.dl</code> file containing declarations and rules. This page covers the building blocks you'll use everywhere.
...
When a variable shows up in multiple atoms of the same rule, it acts as a <b>join key</b> — <StyledFlowLog /> only keeps combinations where that variable has the same value everywhere. In the second rule above, <code>y</code> must match across <code>Path</code> and <code>Edge</code>.
