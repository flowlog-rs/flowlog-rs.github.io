...
<StyledFlowLog /> uses **stratified negation**: the negated relation must be fully computed before the rule that negates it runs. The compiler figures out the right evaluation order automatically. If there's a circular dependency through negation, it'll tell you.
