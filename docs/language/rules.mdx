...
Both <code>Reach(x)</code> and <code>Arc(x, y)</code> are positive atoms. The shared variable <code>x</code> is a join key — <StyledFlowLog /> only matches rows where <code>x</code> has the same value in both relations.
